...
const OfficerLogin: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const response = await api.post(API_ENDPOINTS.OFFICER_LOGIN, { email, password });
      if (response.status >= 200 && response.status < 300) {
        const officerData = response.data.officer || response.data;
        login({ ...officerData, role: 'officer' });
        navigate('/officer/live-detection', { replace: true });
      }
    } catch (error: any) {
      setLoading(false);
    ...
    <div className="login-container">
      <Card className="login-card">
        <div className="login-header">
          ...
          <p className="login-subtitle">Sign in to access officer dashboard</p>
        </div>
        <form onSubmit={handleLogin} className="login-form">
          ...
          <div className="login-links">
            <Link to="/officer/register" className="link">Register as Officer</Link>
          </div>
        </form>
      </Card>
    </div>
